...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (res.ok) {
        setStatus("Şifre sıfırlama bağlantısı gönderildi. E-postanı kontrol et.")
      } else {
        setError(data.message || "Şifre sıfırlama bağlantısı gönderilemedi. Lütfen tekrar deneyin.")
      }
    } catch {
  ...
  const isDisabled = processing || !email
    ...
    <main className="flex min-h-dvh items-center justify-center bg-white dark:bg-zinc-950">
      <div className="mx-auto flex w-full max-w-[360px] flex-col gap-12 p-5 pb-24">
        ...
        <div className="flex flex-col gap-5">
          {status && (
            <p className="text-center text-sm font-medium text-emerald-600">{status}</p>
          )}
          {error && (
