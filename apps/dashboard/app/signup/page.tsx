...
export default function RegisterPage() {
  ...
  const redirectTo = safeRedirectPath(redirectParam) ?? "/onboarding/scan"
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.message || (data.errors ? JSON.stringify(data.errors) : "Kayıt başarısız. Lütfen tekrar deneyin."))
        return
      }
        ...
        <div className="flex flex-col gap-5">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-3.5">
            ...
            >
              {processing ? (
                <>
                  Devam ediliyor... <Loader2 className="ml-2 size-4 animate-spin" />
                </>
              ) : (
                "Devam Et"
              )}
          ...
          <GoogleButton redirectTo={redirectTo} mode="signup" disabled={processing}>
            Google ile devam et
          </GoogleButton>
