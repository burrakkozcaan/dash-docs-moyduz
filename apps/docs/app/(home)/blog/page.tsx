    ...
    <main className="mx-auto w-full max-w-page px-4 pb-12 md:py-12">
      <div className="relative dark mb-4 aspect-[3.2] p-8 z-2 md:p-12">
        <Image
          ...
          priority
          alt="banner"
          className="absolute inset-0 size-full -z-1 object-cover rounded-2xl"
        />
        ...
        <p className="text-sm font-mono text-landing-foreground-200">
          Moyduz ile ilgili en son duyurular.
        </p>
      </div>
      <div className="grid grid-cols-1 gap-2 md:grid-cols-3 xl:grid-cols-4">
        {posts.map((post) => (
          <Link
            key={post.url}
            href={post.url}
            className="flex flex-col bg-fd-card rounded-2xl border shadow-sm transition-colors hover:bg-fd-accent hover:text-fd-accent-foreground overflow-hidden"
          >
            {post.data.image ? (
