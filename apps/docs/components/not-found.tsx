...
export function NotFound(props: NotFoundProps) {
  return (
    <div className="flex flex-col items-center justify-center text-center gap-4 p-8 [grid-area:main]">
      <h1 className="text-4xl font-bold font-mono">Sayfa Bulunamadı</h1>
      <div className="w-full border border-fd-foreground/50 border-dashed p-4 max-w-[600px]">
        <Suspense
          fallback={<p className="text-sm text-fd-muted-foreground">Alternatifler aranıyor…</p>}
        >
          <Alternative {...props} />
        </Suspense>
      </div>
...
async function Alternative({ getSuggestions }: NotFoundProps) {
  ...
  if (suggestions.length === 0) {
    return (
      <div>
        <p className="text-sm text-fd-muted-foreground mb-2">Alternatif bulunamadı</p>
  ...
  return (
    <div>
      <h2 className="text-sm text-fd-muted-foreground mb-2">Belki aradığınız:</h2>
