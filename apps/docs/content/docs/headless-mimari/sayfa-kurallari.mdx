...
Fumadocs generates **page slugs** and **page tree** (sidebar items) from your content directory using [`loader()`](/docs/headless-mimari/kaynak-api),
...
To customise its output further, you may use [Loader Plugins](/docs/headless-mimari/kaynak-api/eklentiler).
...
You can add an [`icon` handler](/docs/headless-mimari/kaynak-api#icons) to `loader()`.
