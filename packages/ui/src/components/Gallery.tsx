...
const defaultImages = Array.from({ length: 10 }, (_, i) => 
  `https://deifkwefumgah.cloudfront.net/shadcnblocks/block/lummi/bw${(i % 10) + 1}.jpeg`
);
...
export function Gallery({
  ...
  useEffect(() => {
    ...
    const animate = (timestamp: number) => {
      ...
      columns.forEach((column, index) => {
        ...
        if (columnHeightsRef.current[index] === undefined || columnHeightsRef.current[index] === 0) {
  ...
  return (
    <section 
      className={`relative min-h-screen overflow-hidden bg-background w-full antialiased ${className}`}
      style={{ 
        isolation: 'isolate',
        contain: 'paint'
      }}
    >
      {/* Gallery columns */}
      <div className="absolute inset-0 flex justify-center gap-6 md:gap-8 overflow-hidden">
              ...
              {duplicatedImages.map((image, imgIndex) => (
                ...
                >
                  <Image
                    ...
                    className="h-full w-full object-cover select-none rounded-xl"
                    draggable={false}
                    style={{ 
                      outline: 'none', 
                      border: 'none', 
      ...
      {/* Gradient overlays - Isolated stacking context with overlap */}
      {/* Top gradient - Mobile */}
      <div 
        className="absolute top-0 left-0 z-50 h-[200px] w-full bg-gradient-to-b from-background via-background to-transparent md:hidden"
        style={{
          top: '-2px',
      ...
      <div 
        className="absolute bottom-0 left-0 z-50 h-[200px] w-full bg-gradient-to-t from-background via-background to-transparent md:hidden"
        style={{
          bottom: '-2px',
          transform: 'translateZ(0)',
          isolation: 'isolate',
      ...
      <div 
        className="hidden md:block absolute top-0 left-0 z-50 h-[300px] w-full bg-gradient-to-b from-background via-background to-transparent"
        style={{
          top: '-2px',
          transform: 'translateZ(0)',
          isolation: 'isolate',
      ...
      <div 
        className="hidden md:block absolute bottom-0 left-0 z-50 h-[300px] w-full bg-gradient-to-t from-background via-background to-transparent"
        style={{
          bottom: '-2px',
          transform: 'translateZ(0)',
          isolation: 'isolate',
      ...
      {/* Left gradient */}
      <div 
        className="absolute top-0 left-0 z-50 h-full w-[80px] bg-gradient-to-r from-background to-transparent md:w-[120px]"
        style={{
          transform: 'translateZ(0)',
          isolation: 'isolate',
          pointerEvents: 'none',
      ...
      {/* Right gradient */}
      <div 
        className="absolute top-0 right-0 z-50 h-full w-[80px] bg-gradient-to-l from-background to-transparent md:w-[120px]"
        style={{
          transform: 'translateZ(0)',
          isolation: 'isolate',
          pointerEvents: 'none',
